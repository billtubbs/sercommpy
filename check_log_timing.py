...
def analyze_log_timing(log_file_path):
    ...
    print(
        "Time differences between consecutive 'Method show_now called.' entries:"
    )
    ...
    max_gap_idx = (
        time_diffs.idxmax()
    )  # Index in original series (includes NaN at 0)
    ...
    print(
        f"Context around the longest gap (lines {start_line + 1} to {end_line}):"
    )
